...
impl Api {
    ...
    async fn metadata(&self, url: &str) -> Result<Metadata, ApiError> {
        ...
        let response = if response.status().is_redirection() {
            let redirect_url = headers.get(LOCATION).unwrap().to_str()?;
            let absolute_url = if redirect_url.starts_with("http://") || redirect_url.starts_with("https://") {
                redirect_url.to_string()
            } else {
                let base = url::Url::parse(url).unwrap();
                base.join(redirect_url).unwrap().to_string()
            };
            self.client
                .get(&absolute_url)
                .header(RANGE, "bytes=0-0")
